    ...
    public abstract class TAnimCore : MonoBehaviour
    {
        ...
        TypewriterCore typewriter
        {
            get
            {
                ...
                {
                    Debug.LogError($"TextAnimatorPlayer component is null on GameObject {gameObject.name}");
                }
#else
        ...
        /// </summary>
        /// <remarks>
        /// PS. this might still contain other tags from different plugins, like "color" from TMPro.
        ...
        public bool allLettersShown
        {
            get
            {
                ...
                for (int i = 0; i < charactersCount; i++)
                {
                    ...
                    {
                        if (characters[i].passedTime < characters[i].info.appearancesMaxDuration)
                            return false;
                    }
                }
        ...
        [SerializeField] string[] defaultAppearancesTags;
        ...
        [SerializeField] string[] defaultDisappearancesTags;
        ...
        protected abstract bool IsCharacterVisibleInRender(int index);
        ...
        void AnimateText()
        {
            ...
            for(int i = 0; i < charactersCount && i<characters.Length; i++)
            {
                ...
                if (!IsCharacterVisibleInRender(i))
                {
                    characters[i].passedTime = 0;
                    characters[i].Hide();
                    continue;
            ...
            if (settings.behaviors.enabled)
            {
                ProcessAnimationRegions(behaviors);
            }

            if (settings.appearances.enabled)
            {
        ...
        [Obsolete("Use TextAnimatorSettings.SetAppearancesActive instead (which will disable effects globally), or remove the database from this script.")]
        public void EnableAppearancesLocally(bool value) => TextAnimatorSettings.SetAppearancesActive(enabled);
        ...
        [Obsolete("Use TextAnimatorSettings.SetBehaviorsActive instead (which will disable effects globally), or remove the database from this script.")]
        public void EnableBehaviorsLocally(bool value) => TextAnimatorSettings.SetBehaviorsActive(enabled);
