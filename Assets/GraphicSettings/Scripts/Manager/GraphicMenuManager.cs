...
namespace AkshayDhotre.GraphicSettingsMenu
{
    ...
    public class GraphicMenuManager : MonoBehaviour
    {
        ...
        private GameObject actualOption;
        [SerializeField] private GameObject applySelected;
        ...
        private void Update()
        {
            actualOption = menuOptions[selector];
            applySelected.SetActive(actualOption == menuOptions[menuOptions.Length - 1]);
            foreach (GameObject option in menuOptions)
            {
                try { option.GetComponent<KeyboardMouseInput>().SetMarkerActive(false); } catch { }
            }
            try { actualOption.GetComponent<KeyboardMouseInput>().SetMarkerActive(true); } catch { }

            if (UIManager.uim.moveDown())
            {
                selector++;
                if (selector > menuOptions.Length - 1)
                {
                    selector = 0;
                }
            }
            if (UIManager.uim.moveUp())
            {
                ...
                if (selector < 0)
                {
                    selector = menuOptions.Length - 1;
                }
            }
            if (UIManager.uim.Interact())
            {
                actualOption.GetComponent<Button>().onClick.Invoke();
            }
        }
